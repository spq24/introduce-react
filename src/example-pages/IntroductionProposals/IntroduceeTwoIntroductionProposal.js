    ...
    if (id) {
      axios.get(`/api/v1/proposed-introduction/${id}`)
        .then(response => {
          ...
          if (intro.completed || intro.introducee_two_rejected || intro.introducee_two_accepted) {
            setHandled(true)
          }
              ...
              <Grid container spacing={6}>
                <Grid item md={12} className="d-flex justify-content-center" style={{ marginBottom: '50px', flexDirection: 'column', alignItems: 'center' }}>
                  <span className="font-size-xl mb-1 text-white" style={{ textAlign: 'center' }}>
                    {`Hey ${introductionProposal.introducee_two.first_name}`},<br /> {`${introductionProposal.introducer.first_name} ${introductionProposal.introducer.last_name} is proposing an introduction to ${introductionProposal.introducee_one.first_name} ${introductionProposal.introducee_one.last_name}.`}
                  </span>
                  <br /><br />
                  <h1 className="font-size-xxl mb-2 font-weight-bold text-white">
                    Will You Accept This Introduction Proposal?
                              ...
                              <span>
                                ...
                                }
                                {
                                  submitted && acceptRejectStatus == 'accepted' ?
                                    `Great! Look out for an email soon that will connect you to ${introductionProposal.introducee_one.first_name} ${introductionProposal.introducee_one.last_name}. We will Cc ${introductionProposal.introducer.first_name} ${introductionProposal.introducer.last_name} as well.` :
                                    acceptRejectStatus !== '' ?
                            ...
                            acceptRejectStatus === 'denied' ?
                              <Grid item md={12}>
                                <TextField
                                  ...
                                  rows={4}
                                  variant="outlined"
                                  name="introducee_two_rejection_reason"
                                  value={introductionProposal.introducee_two_rejection_reason}
